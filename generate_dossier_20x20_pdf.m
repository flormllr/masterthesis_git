function generate_dossier_20x20_pdf(names)
% SUMMARY:
...
%           to add to the dossier20x20.pdf file. If empty, we generate the
%           dossier for all available 20x20-pattern.
...
for i = 1:length(names)
    ...
    if any(pattern == patterns_to_show_original)
        ...
        nexttile;
        plot_cut_list(get_cut_list_patterns_20x20(pattern))
        hold on
        plot([0,0,1,1],[0,1,1,0],'k-')
        hold off
        title("pattern")
    ...
    exportgraphics(fig, "dossier20x20.pdf", 'ContentType', 'image', 'BackgroundColor', 'white', 'Append', true);
    close(fig);
