...
disp("script starts (2x2 on domain)")
...
for p_counter = 1:4
    ...
    % f_orientation = @(orientation_middle) minimize_von_mises_stress_middle(n, n_middle, orientation_middle, p);
    % nvars = n_middle^2;
    % lb = ones(nvars,1);
    % ub = 4*ones(nvars,1)
    % swap_middle = [1,2; 2,1];
        ...
        for i2 = 1:4
            for i3 = 1:4
                for i4 = 1:4
                    ...
                    VMstress(counter) = minimize_von_mises_stress_middle(n, n_middle, [i1, i2; i3, i4], p);
                    counter = counter + 1;
                end
            end
        end
    ...
    name_title = "Min of L" + p + "-norm over entire domain";
    save_pattern_as_dossier_page_pdf(cut_list_min, name_title, "min2x2middle", drawBox, subdomain_to_diplay_normes)
...
save("output/2x2domain_orientation.mat","save_orientation");

disp("We are done, congrats. Please look at your file min2x2middle.pdf")